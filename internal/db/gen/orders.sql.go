...
func (q *Queries) CountOrderItems(ctx context.Context, orderID pgtype.UUID) (int64, error) {
...
const countOrdersForDay = `-- name: CountOrdersForDay :one
SELECT COUNT(*)
FROM orders
WHERE created_at >= $1 AND created_at < $2
`

type CountOrdersForDayParams struct {
	StartAt pgtype.Timestamptz `json:"start_at"`
	EndAt   pgtype.Timestamptz `json:"end_at"`
}

func (q *Queries) CountOrdersForDay(ctx context.Context, arg CountOrdersForDayParams) (int64, error) {
	row := q.db.QueryRow(ctx, countOrdersForDay, arg.StartAt, arg.EndAt)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
RETURNING id, user_id, cart_id, status, currency, pricing_subtotal, pricing_discount, pricing_tax, pricing_shipping, pricing_total, shipping_address, shipping_option, notes, created_at, updated_at, applied_voucher_code, tenant_id, order_number
`
...
SELECT id, user_id, cart_id, status, currency, pricing_subtotal, pricing_discount, pricing_tax, pricing_shipping, pricing_total, shipping_address, shipping_option, notes, created_at, updated_at, applied_voucher_code, tenant_id, order_number
FROM orders
WHERE id = $1
LIMIT 1
`
...
const getOrderThumbnail = `-- name: GetOrderThumbnail :one
SELECT pi.url
FROM order_items oi
...
func (q *Queries) GetOrderThumbnail(ctx context.Context, orderID pgtype.UUID) (string, error) {
	row := q.db.QueryRow(ctx, getOrderThumbnail, orderID)
	var url string
	err := row.Scan(&url)
	return url, err
}
