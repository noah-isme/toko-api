...
// CreateDraft creates a draft order without payment intent.
func (s *Service) CreateDraft(ctx context.Context, userID *string, in Input) (Output, error) {
	if s == nil || s.Q == nil || s.Pool == nil {
		return Output{}, errors.New("checkout service not configured")
	}
	if userID == nil || *userID == "" {
		return Output{}, errors.New("user is required for checkout draft")
	}
	if in.CartID == "" {
		return Output{}, errors.New("cartId is required")
	}
