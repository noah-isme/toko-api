...
func Load() (*Config, error) {
	...
	cfg := &Config{
		...
		SMTPHost:                 strings.TrimSpace(k.String("SMTP_HOST")),
		SMTPPort:                 parsePositiveInt(k.String("SMTP_PORT"), 587),
		SMTPUsername:             k.String("SMTP_USERNAME"),
		SMTPPassword:             k.String("SMTP_PASSWORD"),
		// Falls back to NOTIFY_FROM_EMAIL so a deployment only has to set the
		// sender address once.
		SMTPFrom:                   valueOrDefault(k.String("SMTP_FROM"), valueOrDefault(k.String("NOTIFY_FROM_EMAIL"), "no-reply@toko.local")),
		SMTPImplicitTLS:            parseBool(k.String("SMTP_IMPLICIT_TLS")),
		SMTPAllowInsecureTLS:       parseBool(k.String("SMTP_ALLOW_INSECURE_TLS")),
		SMTPTimeout:                time.Duration(parsePositiveInt(k.String("SMTP_TIMEOUT_SEC"), 10)) * time.Second,
